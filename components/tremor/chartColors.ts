...
// Every class name below is written out as a literal string (not built via
// template interpolation) so Tailwind's static scanner can see it and
// generate the matching CSS — a computed `fill-${color}-500` string would
// never appear as literal text in this file, so Tailwind would silently
// skip generating that utility and the chart segments would render with no
// fill at all.
const chartColors: Record<AvailableChartColorsKeys, Record<ColorUtility, string>> = {
...
/** Cycles `colors` across `categories` positionally, same category always gets the same color. */
